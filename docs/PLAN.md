...
- [x] 缺陷收口 2.1.4（**当前版本 2.1.4**，2026-09-20：渲染→主进程边界收紧、截图引擎不再被一次启动失败锁死、换地址清掉对应设备旧截图、截图任务互斥 + 删死文件。四道门：出包 ✅ / 推 origin ✅ / tag ✅ / 装机复验 ✅；**GitHub Release 未挂，卡在 `gh` 在本机无任何凭据**（`%APPDATA%\GitHub\` 不存在、`GH_TOKEN` 未设），说明稿与 SHA256 已备好）
- [ ] 下一步：把 2.1.4 的 Release 补上（挂 `release/PreviewCraft-Setup-2.1.4.exe` 并匿名核验字节数与下载链路）；其后 Backlog 择机（长截图 / 登录态截图 / 批量队列 / OG 预设 / 托盘 / 导出历史），以及 16px 图标简化版与 Chromium 真取消（0.5~1 天）——用户 2026-09-20 定：功能清单先记在 §7，有空再做
