...
captureStart(input: { url: string; deviceUrls: Partial<Record<DeviceId, string>>; devices: DeviceId[]; replace?: Partial<Record<DeviceId, string>> }): Promise<CaptureResult>   // 事件 capture:progress → { device, status: 'pending'|'done'|'error', path? , error? }
                                                                                                                    // replace = 本轮要替换掉的旧截图路径，主进程开截前删掉（2.2.1）
...
sessionGet(): Promise<SessionSnapshot | null>; sessionSet(snap: SessionSnapshot): Promise<void>   // 上次会话快照，key: session:v1（2.2.1）
cacheStats / cacheClear / cacheOpen                                               // 临时产物统计、全量清除、在资源管理器里打开目录
```

**通道清单以 `src/shared/types.ts` 的 `Api` 接口为准**（冒烟 B 段逐条断言 bridge 方法齐备，当前 29 个）。
...
- [x] 视觉迭代 2.2.0（2026-09-20 用户看图签字通过：背景板 7 → 15 块重做、五套预设按构图口径重排、阴影按显示宽分级、desktop 壳去粗支架 + mobile 灵动岛 + 机身轮廓光、缩略图对齐真机、背景板选择器换 HeroUI 内置 `ColorSwatchPicker`；同日续做 **28 处按钮补图标（lucide-react）** 与两处收口（缩略图贴边、圆角默认 12 连带 JPG 垫白）。门禁 typecheck / lint / build 零错误，冒烟 **133 条**（新增 10 条：构图光学判据 ×3、壳体几何 ×3、色板代表色 ×2、内置选择器交互、圆角 JPG 垫白 ×1）。**版本 2.2.0 已发布：出包 / 推 origin / tag+Release / 装机复验四道门全过（2026-09-20），安装包 116,572,866 字节匿名核验一致、装机后 `ProductVersion=2.2.0.0`、打包版冒烟 135/135**）
- [x] 技术债收口 2.2.1（2026-09-20 用户定「把技术债和当前遗漏的问题全部解决，改完测试通过发个新包」：§7 挂出的 **7 项全部清账** —— 五项功能面债（缓存只增不减 / 导出监听器与定时器泄漏 / setState 里发 IPC / 会话不落盘 / 截图倍率语义）+ renderer 专用依赖移出 asar + 冒烟「未污染真实设置」识别并发实例。门禁 typecheck / lint / build 零错误，源码态冒烟 **139/139 PASS 零 SKIP**、打包态 **137/137 + 2 SKIP**（同日续作把内嵌拦截分支改打本地夹具站点，改后两种模式均 **139/139 零 SKIP**，见下一行）；`app.asar` 55,708,235 → **14,932,587 字节（−40.8 MB / −73%）**、安装包 **113,145,723 字节**、装机落盘 421 MB → **382 MB**。四道门全过：出包 ✅ / 推 origin ✅ / tag + Release ✅ / 装机复验 ✅（含会话恢复端到端实测））
- [x] 2.2.1 续作二：视觉项补常驻断言（2026-09-21：图标此前只在 2.2.0 出包时由一次性脚本核过、脚本删掉后这条覆盖面就没了，现补进冒烟 B 段 **3 条** —— 顶栏四枚按钮各带一枚 lucide 图标、图标实测渲染成 16×16（未被压成 0）、全站 `button svg` 无写死 Tailwind 尺寸类；顺带改掉 §5 里「README 配图仍是 2.2.0 口径」那句易误读表述。门禁 typecheck / lint / build 零错误，冒烟源码态与打包态均 **142/142 PASS 零 SKIP**（打包态第一轮撞出 GitHub 对 mobile 那台回了「Access to this site has been restricted」限流页 → 16KB 被体积判据抓红，重跑即绿，非应用缺陷）。纯测试 + 文档，不动应用代码、不升版不出包）
- [ ] 下一步：Backlog 择机（长截图 / 登录态截图 / 批量队列 / OG 预设 / 托盘 / 导出历史），以及 16px 图标简化版与 Chromium 真取消（0.5~1 天）；shots 系四条（浏览器窗口壳 / 画布比例族 / 一台多屏 / Tilt）见 §7，其中「浏览器窗口壳」属方案级、动手前先记 §六 待确认。§7 仅剩一项：停帧环境事件只能定性不能自救（单人自用不值得为此建 CI，建议当已知边界长期挂着）
...
| 2026-09-20 | 2.2.0 发布收口（**四道门全过**） | 关闭上一条遗留 ① 与 ②。**门 2 推送**：`git push origin main` `bc49439..40a3cb7`（10 个 commit，走 SSH 不受 github.com HTML 端点影响）；annotated tag `2.2.0` 已推（说明稿沿用 2.1.x 分条格式）。**门 3 一次撞出四条新环境坑，都值得记死**：① `gh release create … \| tail -5` 接进管道后**退出码取自 tail**，gh 其实失败了但被报成 exit 0，Release 根本没建成 —— 是 `api /releases` 列出 8 条、最新仍是 2.1.4 才发现；2.1.4 记过的「别把命令接进管道」在**后台任务**里以同样方式复现，后台任务要重定向到文件而不是接管道。② 本机 `gh` 的**网络类子命令静默返回空**（`gh release list` / `gh repo view` 均 exit 0 无输出，而 `gh --version`、`gh auth status` 正常，同一时刻 curl 打 API 稳拿 200）—— 发布写操作因此改走 `curl` + REST：`gh auth token` 取令牌写进 curl 配置文件（不进命令行、不落日志，用完即删）→ `POST /repos/.../releases` → `POST uploads.github.com/repos/.../releases/<id>/assets?name=`。③ `X-GitHub-Api-Version: 2022-08-04` 被拒 400（当前只支持 `2026-03-10` 与 `2022-11-28`），**这个头干脆不发**。④ 路由复核：`uploads.github.com` 裸连 0.53s、走系统代理 2.51s → 与 api.github.com 同口径**裸连**。**Release** <https://github.com/baiwumm/preview-craft/releases/tag/2.2.0>：release id 392355601、附件 id 576398366、`bytes_uploaded=116,572,866` 全量（均速 7.4 MB/s）；**匿名核验** `draft=false`、附件字节数 = 本地 `stat`、公开下载链路 `302 → 206` 且 `Content-Range: bytes 0-15/116572866`、首两字节 `4d 5a`（MZ，真在伺服 PE）。**门 4 装机复验通过**：装前 `Get-Process PreviewCraft` 判 `NO_RUNNING_INSTANCE`（不顶掉用户窗口）→ `Setup-2.2.0.exe /S` 静默覆盖安装 16s → exe `ProductVersion=2.2.0.0`（从 2.1.4.0 升上来）、卸载项 `DisplayName=PreviewCraft 2.2.0 / DisplayVersion=2.2.0`（键名仍是 appId 派生 GUID `4cf14dc5-…`）、桌面与开始菜单快捷方式齐、安装目录 421MB、`%APPDATA%\preview-craft` 保留 → **在 `release/win-unpacked/PreviewCraft.exe` 上跑全量冒烟 135/135 PASS、零 SKIP**（上一条的停帧环境事件本轮未复现，窗口保持可见即可跑通）。**再记一条 PowerShell 坑**：`.ps1` 存成**无 BOM 的 UTF-8 又带中文注释**时，Windows PowerShell 5.1 按 GBK 解码，中文串会吞掉引号报「字符串缺少终止符」——核验脚本改纯 ASCII 才跑得起来，与 2.1.4 记的「Git Bash 把 `$p` 当变量展开」是两条独立成因 | ① 本轮发布收口的 docs 提交是否推 origin，单独待放行；② §7「renderer 专用依赖挪 devDependencies」候选仍未动（需实测安装版启动）；③ 五项已报出技术债继续候排期 |
| 2026-09-20 | 技术债收口（v2.2.1，**四道门全过**） | 用户定「把技术债和当前遗漏的问题全部解决，改完测试通过发个新包」。**先排清单再动手**：§7 七项一次性收口。**① 依赖归类**：查 `electron.vite.config.ts:11` 确认主进程用 `externalizeDepsPlugin({ exclude: ['electron-store'] })`，即 electron-store 已被打进 out/main，它在 asar 里那份是死重；实测 `out/main/index.js` 运行时只 require `@puppeteer/browsers` / `electron` / `puppeteer-core`，于是把 @heroui/react / @heroui/styles / react / react-dom / tailwind-variants / electron-store 六者全挪 `devDependencies`，`dependencies` 只剩两个 puppeteer。**asar 55,708,235 → 14,926,431（−40.8 MB / −73%）**，安装包 116,572,866 → 113,142,952（只降 3.3 MB，因为 NSIS 里这部分本就被 LZMA 压掉十倍以上、大头是 Electron 运行时）；打包版冒烟 133/133 先验不白屏，改完 src 再跑一次 137/137 + 2 SKIP。**② 缓存回收**：`CaptureStartInput` 加 `replace` 字段（渲染进程把本轮要替换的旧截图路径交下来，主进程开截前删，路径须在缓存目录内）+ `sweepCache` 容量兜底（>400 MB 按 mtime 从旧往新删，**1 小时内的产物一律不碰**，因为可能正被画布引用）+ `cache:open` 与「打开缓存目录」按钮。**③ 监听器泄漏**：`export:ready` / `export:webp:result` 改 `onceFromWindow`（具名 handler、成功与超时两条路径都摘、按 `event.sender` 过滤），`withTimeout` 补 `clearTimeout`，`ExportPage` 解码轮询加 cleanup 与 15s 上限（到点即停，让主进程 20s 硬超时把错误抛给用户，绝不在缺图时谎报就绪）。**④ 红线那条**：另存/删除自定义模板改「先 await IPC 再 setState」，失败补 Toast 且不再把自己设为排版来源。**⑤ 会话落盘**：`session:v1` + `session:get/set` 通道，读时做最小形状校验（磁盘上可能是旧版结构，宁可当作没会话也不要启动即崩）、写防抖 800ms、**启动恢复完成前不落盘**（否则把默认值写回去）、恢复优先于「默认模板/默认背景」设置（hint 文案同步改）；UrlBar 加 `restoreSession` 把主地址与分设备草稿一并回填并走正常提交链路。**⑥ 倍率语义**：`deviceScaleFactor: 2` 提成具名 `CAPTURE_DEVICE_SCALE`，导出面板说明「截图固定 2x 采集、导出倍率是合成整张画布时的放大，相乘才是最终像素」。顺带把 `StyleState` 从 renderer 私有挪进 `@shared/types`（三层共用契约）。**冒烟新增 4 条断言 + 12 个通道**，源码态 **139/139 零 SKIP**。**自己造的一处坑要记**：「重截会删旧图」这条断言原先放在导出段之前，把后面还要复用的 `cap.shots` 文件删了 → 11 条导出断言连锁 ENOENT，而它本身 PASS（说明应用行为是对的、测试顺序错了）；挪到所有导出断言之后才对。**停帧自救的实测结论**：加了 `Page.setWebLifecycleState` + `getAnimations().finish()` 两级自救后，分支真被触发过 —— 但**对已 hidden 的窗口 setWebLifecycleState 拉不回来、finish() 也等不到 React 卸载**，所以仍只能 FATAL 定性；中途试过第三级「把滞留遮罩节点从 DOM 摘掉」，实测会在保存真正落地前就把弹窗摘掉，把一个清晰 FATAL 变成一串迷惑 FAIL，**已撤回**，不给测试加假绿。**发布**：版本升 2.2.1 → `pnpm dist` 出 **113,145,723 字节**、SHA256 `65cd703ed5b90f3311a6045a6558d39da9a077c7a18eacafb9b758a0dbde3dee` → 按 hygiene 规则删掉本地被取代的 2.2.0 包（顺带消除「本地 2.2.0 是更瘦的重建、与已发布 2.2.0 字节不一致」的隐患）→ 打包卫生复验 asar 头部 `lucide-react`/`@heroui/react`/`react-dom`/`.smoke-out`/`.smoke-profile`/`.agents`/`PLAN.md` **全部未命中**、`puppeteer-core` 命中（运行时真需要）→ 推 `831b1cf..e60e66b`（4 个 commit：chore 依赖 / fix 五债 / test 冒烟 / chore 版本）→ annotated tag `2.2.1` → Release <https://github.com/baiwumm/preview-craft/releases/tag/2.2.1> 挂包（release id 392393876、asset id 576611849），匿名核验 `draft=false`、附件 113,145,723 字节 = 本地、Range **206** 且 `Content-Range: bytes 0-15/113145723`、首两字节 `4d 5a`（MZ） → 装机 `2.2.0.0 → 2.2.1.0`（12s，装前确认 `NO_RUNNING_INSTANCE`）、卸载项与两类快捷方式齐、**安装落盘 421 MB → 382 MB**、装后 `app.asar` 14,932,587 与构建一致 → 再用上一轮冒烟留下的隔离档案启一次打包版做**会话恢复端到端实测**：档案里 `mainUrl=https://github.com/ / template=classic / radius=15`，重启后地址栏回填、画布 4 台渲染、画廊选中「经典全家福」✓ | ① §7 当时仍留两项，其中「冒烟拦截分支依赖外网」已在下一行续作里清掉，只剩停帧定性一项；② README 两张配图**与当前界面一致**（2.2.0 那轮已随图标与默认圆角重截，2.2.1 零观感变化故未再重拍；原表述「仍是 2.2.0 口径」易误读成过期，已改）；③ 视觉项断言缺口已补，见下一行续作 |
| 2026-09-20 | 冒烟续作：内嵌拦截分支不再依赖外网 | 用户点头清 §7 剩两项里的第一条。先做可行性判定再动手：读 `src/shared/url.ts` 确认 `normalizeUrl` 只要求 hostname 含点（`127.0.0.1` 通过），读 `src/renderer/index.html` 确认 CSP 是 `frame-src http: https:`（本地 http iframe 放行），读 `src/main/probe.ts` 确认探测只看响应头、不挑状态码 —— 三条都成立才动。**做法**：冒烟内起一个 `node:http` 夹具服务（`listen(0,'127.0.0.1')` 拿随机端口），`/deny` 回 `X-Frame-Options: DENY`、`/open` 不带任何内嵌相关头；三条断言全部改打本地地址 —— 判定（`probed:true / blocked:true / reason:'X-Frame-Options: DENY'`）、放行（`probed:true / blocked:false`）、**占位渲染**（把 `/deny` 贴进地址栏 → 画布出现「该站点禁止内嵌预览」）。第三条是本轮真正的增量：它验的是「探测结果 → `embedHints` → `DeviceFrame` 占位」这段端到端接线，此前只能借 github.com 的真实头验，一抖就 SKIP。服务在 `finally` 里关；踩到一条 JS 作用域坑 —— `const` 声明在 `try` 块内、`finally` 取不到，必须把 `let embedServer` 提到 `try` 外面。**结果**：源码态 **139/139 PASS、零 SKIP**，打包态同样 **139/139 零 SKIP**（原先 137 + 2 SKIP，那两条现在必然执行、总数涨到 139），且这三条从此不可能被静默跳过。**顺带又撞一次 §7 剩的那条停帧事件**：第一次跑在「另存为模板」处窗口掉 hidden 抛 FATAL（三条新断言在此之前已全部 PASS），重跑即绿 —— 再次确认那两级自救对已 hidden 的窗口无效，这条按约定不做、继续挂着 | ① §7 仅剩停帧事件一项（本轮又实测触发一次，证据充分）；② 纯测试改动，不动应用代码、不升版不出包 |
| 2026-09-21 | 2.2.1 续作二：视觉项补常驻断言 | 用户追问「技术债是否全清了」，**不靠印象回答**：逐行扫 §5 近几轮的「遗留」列，撞出自己上一轮写过却没关的一条 —— 2.2.0 给 28 处按钮补图标时，核验是靠**出包时的一次性脚本**做的（§5 2.2.0 出包行记着「按钮内 8 枚 SVG 带 `class="lucide lucide-refresh-cw"`」），脚本随轮次删掉后这条覆盖面就是空的；在 `scripts/smoke.mjs` 里 grep `svg` 与 `lucide` **零命中**（同期「圆角」有 11 处命中）坐实。**补三条 B 段常驻断言**：① 顶栏四枚按钮各带一枚 `svg.lucide`；② 图标 `getBoundingClientRect()` 实测 **16×16**（HeroUI Button 的 CSS 给的尺寸），落在 12~26px 且必须正方，防「svg 在但被压成 0 高」这种不报错、只会悄悄退回纯文字的坏法；③ 全站 `button svg` 无一处写死 Tailwind 尺寸类（`size-N` / `w-N` / `h-N`），因为那会和 Button 的 CSS 打架。**选择器按 aria-label / 文案逐枚点名，不用 `header button svg` 计数** —— 顶栏手风琴触发器也是 `button`、HeroUI 的指示箭头也是 `svg`，计数口径会被它带偏（这是写的时候先撞出来的假红）。**门禁**：typecheck / lint / build 零错误，源码态 **142/142 PASS 零 SKIP**。**打包态第一轮 141/142**，唯一 FAIL 是 `截图 mobile 尺寸符合 2x 预设 — 780x1680 16KB`（其余三台 601/459/941KB 正常）：把那张 PNG 从 `.smoke-out/` 捞出来看，画面是 GitHub 的 **「Access to this site has been restricted.」** 限流页 —— 尺寸合规、内容不是目标站，**断言按设计抓住了它，不是应用缺陷也不是渲染问题**（同一台同一视口，重跑即回 335KB）；打包态**重跑 142/142 零 SKIP**，三条新断言在包内产物上同样通过。**没有为了让它绿而给这条加 skip/soft 分支**：真出空白截图时正需要它红。**文档**：§5 2.2.1 行遗留 ② 的「README 两张配图仍是 2.2.0 口径」改为「与当前界面一致（2.2.0 那轮已随图标与默认圆角重截，2.2.1 零观感变化故未再重拍）」—— 原表述会被读成配图过期；遗留 ③ 同时关闭。纯测试 + 文档，不动应用代码、不升版不出包。**出包与否已当面对齐**：`git diff --name-only 2.2.1 HEAD` 只有 `docs/PLAN.md` 与 `scripts/smoke.mjs`，而 `electron-builder.yml` 的 `files` 排除项含 `!docs/**` 与 `!scripts/**`，即新包 `app.asar` 会与已发布的 2.2.1 逐字相同 —— 用户定**本轮不出包**，装机版 2.2.1 即最新（不重跑 `dist` 也顺带避开了「本地重建包与已发布同版本字节不一致」那个 2.2.0 记过的坑） | ① 上游限流页这类「尺寸对但内容不是目标站」的抓取，冒烟只有 20KB 体积这一道间接防线，不做文案级识别（GitHub 的拦截页会变，写死匹配串只会变成新的假绿源）；② §7 仍只剩停帧事件一项 |
...
**2026-09-20（2.2.1）已清账 7 项**，细节见 §5 对应执行记录：导出 IPC 监听器与定时器泄漏（`onceFromWindow` 双路径摘除 + 按 sender 过滤）、截图缓存只增不减（`CaptureStartInput.replace` + `sweepCache` 容量兜底 + 「打开缓存目录」入口）、会话状态不落盘（`session:v1` + 防抖落盘 + 启动恢复）、`setState` 更新函数里发 IPC（改 await 后再 setState，并加静态判据断言）、截图倍率语义（提具名 `CAPTURE_DEVICE_SCALE` + 面板说明）、renderer 专用依赖被打包进 asar（六者挪入 `devDependencies`，asar −40.8 MB）、「未污染用户真实设置」分不清污染与并发使用（识别非冒烟实例即走 SKIP 并写明占用者）。

**2026-09-20（同日续）再清 1 项：冒烟的内嵌拦截分支不再依赖外网** —— 见下方 §5 执行记录。

**2026-09-21 复核**：用户问「技术债是否全清了」，逐行扫 §5 各轮「遗留」列后补清一项——2.2.0 那轮给按钮加的图标只被一次性的出包核验脚本验过，脚本随轮次删掉后这条覆盖面就是空的（`grep svg scripts/smoke.mjs` 零命中），已补 3 条常驻断言（详见 §5 最后一行）。至此**本节除下面这一条外全部清零**。

仍待排期：

- **停帧环境事件只能定性、不能自救**：窗口掉到 `visibility=hidden` 后，CDP `Page.setWebLifecycleState` 拉不回来、`getAnimations().finish()` 也等不到 React 卸载，冒烟只能抛 FATAL 定性（详见 §5 2.2.1 行；本轮又一次实测触发，两条自救均无效，重跑即绿）。要真正可恢复，剩下的候选是在启动被测实例前确认桌面会话活跃，或在 CI 里跑虚拟桌面。单人自用工具不值得为此建 CI，建议作为已知边界长期挂着。
