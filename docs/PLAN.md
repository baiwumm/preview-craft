...
- [x] 技术债收口 2.2.1（2026-09-20 用户定「把技术债和当前遗漏的问题全部解决，改完测试通过发个新包」：§7 挂出的 **7 项全部清账** —— 五项功能面债（缓存只增不减 / 导出监听器与定时器泄漏 / setState 里发 IPC / 会话不落盘 / 截图倍率语义）+ renderer 专用依赖移出 asar + 冒烟「未污染真实设置」识别并发实例。门禁 typecheck / lint / build 零错误，源码态冒烟 **139/139 PASS 零 SKIP**、打包态 **137/137 + 2 SKIP**；`app.asar` 55,708,235 → **14,932,587 字节（−40.8 MB / −73%）**、安装包 **113,145,723 字节**、装机落盘 421 MB → **382 MB**。四道门全过：出包 ✅ / 推 origin ✅ / tag + Release ✅ / 装机复验 ✅（含会话恢复端到端实测））
- [ ] 下一步：Backlog 择机（长截图 / 登录态截图 / 批量队列 / OG 预设 / 托盘 / 导出历史），以及 16px 图标简化版与 Chromium 真取消（0.5~1 天）；shots 系四条（浏览器窗口壳 / 画布比例族 / 一台多屏 / Tilt）见 §7，其中「浏览器窗口壳」属方案级、动手前先记 §六 待确认。§7 仍留两项：冒烟拦截分支依赖外网、停帧事件只能定性不能自救
...
- **冒烟的两条拦截分支依赖外网**：本机到 github.com 常 10s 上下顶到 8s 探测超时，近几轮里反复出现 2 条 SKIP（2.2.1 那轮网络争气，零 SKIP 跑满 139 条）；要满覆盖得在 CI 里造一个可控的「拒绝内嵌」站点。
- **停帧环境事件只能定性、不能自救**：窗口掉到 `visibility=hidden` 后，CDP `Page.setWebLifecycleState` 拉不回来、`getAnimations().finish()` 也等不到 React 卸载，冒烟只能抛 FATAL 定性（详见 §5 2.2.1 行）。要真正可恢复，剩下的候选是在启动被测实例前确认桌面会话活跃，或在 CI 里跑虚拟桌面。
