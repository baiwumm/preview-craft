...
4. **发布事务**（git tag / GitHub Release）：tag 与装机均已就绪 —— 2.x 线首个 annotated tag `2.1.2` 已推 origin（此前仅有废弃 Web 版 1.0.0~1.3.0），2.1.2 安装包已完成装机与卸载实测、安装版内冒烟 63/63。**唯一剩的动作是「把安装包挂到 GitHub Release」**：本机 `gh` 未装（MSI 需 UAC 提权，非交互装不上），二选一 —— 用户本机 `winget install GitHub.cli` + `gh auth login` + `gh release create 2.1.2 release\PreviewCraft-Setup-2.1.2.exe`，或直接网页 `releases/new?tag=2.1.2` 拖拽上传。仓库 public，挂上去即公开可下载。
